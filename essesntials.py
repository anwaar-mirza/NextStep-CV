...
<Instruction>Indent all sub-bullets (e.g., under job titles in Experience) with EXACTLY 9 spaces before the dash (-).</Instruction>
<Instruction>Education and Experience must be in reverse chronological order, each entry starting with 5 spaces before the bullet (•) followed by bold degree/role title, institution/company, location, and date range.</Instruction>
<Instruction>Projects must be separate from experience; each project starts with 5 spaces before the bullet (•) and a bold title followed by a short description.</Instruction>
<Instruction>All other information (skills, programming languages, tools, certifications, languages) goes between Projects and Reference in a single combined block, using 5 spaces before each bullet (•).</Instruction>
